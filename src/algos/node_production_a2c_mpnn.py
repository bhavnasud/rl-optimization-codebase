...
class Actor(torch.nn.Module):
    ...
    def __init__(self, node_size=4, edge_size=0, hidden_dim=32, out_channels=1):
        ...
        self.last_layer = nn.Linear(node_size + hidden_dim, out_channels)
...
class A2C(nn.Module):
    ...
    def forward(self, obs, jitter=1e-20):
        ...
        # actor: computes concentration parameters of a X distribution, and desired
        # production amount
        # first element of output is desired production amount
        actor_output = self.actor(graph.x, graph.edge_index, graph.edge_attr)
        desired_production = actor_output[0]
        # the rest of the output can be interpreted as parameters of Dirichlet distribution
        concentration = F.softplus(actor_output[1:]).reshape(-1)
        concentration += torch.rand(concentration.shape) * jitter

        # critic: estimates V(s_t)
        value = self.critic(graph.x, graph.edge_index)
        return desired_production, concentration, value
    
    def select_action(self, obs, deterministic=False):
        desired_production, concentration , value = self.forward(obs, jitter=0 if deterministic else 1e-20)
        print("desired production ", desired_production)
        print("concentration ", concentration)
        if deterministic:
            action = (concentration) / (concentration.sum())
            return desired_production, list(action.cpu().numpy()) 
        else:   
            # TODO: figure out how to get log prob from desired production + sampled action
