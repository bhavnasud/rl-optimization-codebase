...
class A2C(nn.Module):
    ...
    def training_step(self):
        ...
        for r in self.rewards[::-1]:
            ...
            R = r + args.gamma * R
            returns.insert(0, R)
        #     self.all_returns.append(R)
        # if len(self.all_returns) > 10000:
        #     self.all_returns = self.all_returns[-10000:]
        ...
        if (len(returns) == 1):
            ...
            std_dev = returns.std()
        # TODO: maybe instead normalize by reward of shortest path?
        returns = (returns - returns.mean()) / (std_dev + self.eps)
        for (log_prob, value), R in zip(saved_actions, returns):
            print("log prob ", log_prob, "R ", R, "value ", value.item())
